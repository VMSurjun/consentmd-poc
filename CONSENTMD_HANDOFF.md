...
7. **Gaze/attention-verified reading, gating the PDF.** The guided reading mode (added after the initial hackathon build — see `consentmd_poc.html`, "Walk me through it") currently paces each disclosure by estimated reading time on a fixed timer. The next step is verifying attention rather than just pacing it: webcam-based gaze estimation (e.g. WebGazer.js) gating the "Continue" control on sustained dwell-time per paragraph, so the signed PDF is only memorialized once every disclosure has been attention-verified, not just time-elapsed. Practical notes for whoever picks this up:
   - Webcam gaze tracking is reliable at "attention on screen vs. looked away," not literal word-level tracking — frame it that way to avoid overclaiming.
   - Collecting gaze/biometric data triggers its own consent requirement (e.g. Illinois BIPA and similar biometric privacy laws) — a real wrinkle worth having an answer for, given the product's whole premise is informed consent.
   - The natural implementation hook is an attestation line stamped into the existing AcroForm PDF (e.g. "Patient attention-verified across all N disclosures via on-screen gaze tracking, [timestamp]") rather than restructuring PDF generation.
